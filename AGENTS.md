...
- **访客地图**：`_includes/footer.html` 由 `assets/js/footer.js` 自动在沙箱 iframe 中加载 `mapmyvisitors.com/map.js`；外观保持原来的灰度地图，iframe 用来隔离地图脚本连带的旧版 jQuery，不显示加载按钮。srcdoc 内的脚本必须保留 `id="mapmyvisitors"`，第三方脚本会用这个 ID 定位自身配置。修改 `assets/js/footer.js` 后同步递增 `_config.yml` 的 `asset_versions.footer`，避免线上继续命中旧脚本缓存。
