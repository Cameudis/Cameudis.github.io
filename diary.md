...
- 在迭代 $k_i$ 的过程中，有一步是 $B=Table[|k_i|]$。攻击者可以将变量 B 控制在物理内存的一个固定位置，然后预先计算出所有可能的 `Table[|k_i|]` 的值与对应的密文，这样就可以通过观察 B 的密文来恢复出每个 $|k_i|$！当然，这个过程需要一些工程操作，比如需要在循环每过一次之后就重置一遍内存把 B 从缓存里踢出去，这样才能确保每次更新 B 都会写入到内存中。总之恢复了所有 $|k_i|$ 以后就能恢复出 k 了（因为是绝对值所以可能需要一点爆破）。
